...
import type { CartItem, Service, CategoryWithServices, SubService, Location } from './components/types';
import { CheckoutPage } from './components/CheckoutPage';
import api from '@/lib/api';
import toast, { Toaster } from 'react-hot-toast';
import { useAppDispatch, useAppSelector } from '@/app/store/hooks';
import { addToCart, updateQuantity, clearCart } from '@/app/store/features/cart/cartSlice';
...
const CheckCircleIcon = () => (<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-500 mr-2 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor"> <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /> </svg>);
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[95vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                ...
                <main className="overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-4 bg-green-50 border border-green-200 rounded-lg"> <h3 className="font-semibold text-green-800 mb-3">What's Included?</h3> <ul className="space-y-2 text-sm text-gray-700"> {service.inclusions.map((item, i) => <li key={i} className="flex items-center"><CheckCircleIcon />{item}</li>)} </ul> </div>
                        <div className="p-4 bg-red-50 border border-red-200 rounded-lg"> <h3 className="font-semibold text-red-800 mb-3">What's Excluded?</h3> <ul className="space-y-2 text-sm text-gray-700"> {service.exclusions.map((item, i) => <li key={i} className="flex items-center"><XCircleIcon />{item}</li>)} </ul> </div>
                    </div>
